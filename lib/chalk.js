...
chalk.sprint = function(s, hex) {
  const color = chalk.use16m ? style.color.ansi16m(style.hexToRgb(hex))
                             : chalk.use256 ? style.color.ansi256(style.hexToAnsi256(hex))
                                            : style.color.ansi(style.hexToRgb(hex));
...
chalk.init = function() {
  for (let f of file.listCodeDir('colors')) {
    ...
    for (let x of _.pairs(data)) {
      ...
      if (chalk.use16m) {
        theme[k] = style.color.ansi16m(style.hexToRgb(v));
        theme[bgK] = style.bgColor.ansi16m(style.hexToRgb(v));
